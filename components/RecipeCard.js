...
export default function RecipeCard({ recipe }) {
  ...
  const formattedRecipe = {
    ...
    title: recipe.title,
    image: recipe.image,
    summary: recipe.summary || `${recipe.title} - A delicious recipe made with your ingredients.`,
    usedIngredientCount: recipe.usedIngredientCount || 0,
    missedIngredientCount: recipe.missedIngredientCount || 0,
    readyInMinutes: recipe.readyInMinutes || 20,
    servings: recipe.servings || 2,
    likes: recipe.likes || 0,
    ingredients: recipe.usedIngredients?.map(i => i.name) || recipe.ingredients || [],
    instructions: recipe.instructions || ["Loading instructions..."],
    extendedIngredients: recipe.extendedIngredients || recipe.usedIngredients?.map(i => ({ 
      original: i.name 
    })) || []
  };
        ...
        <View className="bg-white rounded-lg overflow-hidden shadow-lg">
          ...
          <View className="p-4">
            ...
            <View className="flex-row justify-between items-center">
              <View>
                <Text className="text-sm text-gray-600">Match: {matchPercentage}%</Text>
              ...
              <View className="items-center">
                ...
                <Text className="text-sm font-semibold">{recipe.likes || 0}</Text>
              </View>
            </View>
          </View>
        </View>
